...
class Configuration(object):
    ...
    def __init__(self, dep_graph, feature_extractor):
        ...
        """
        # print "transitionparser Configuration __init__ 1"
    ...
    def extract_features(self):
        ...
        :return: list(str)
        """
        # print "transotionparser::class Configuration extract_features 1"
...
class TransitionParser(object):
    ...
    def __init__(self, transition, feature_extractor):
        # print "transitionparser __init__ 1"
        self._dictionary = {}
        self._transition = {}
    ...
    def parse(self, depgraphs):
        ...
        """
        # print "transitionparser parse 1"
        result = []
        if not self._model:
            raise ValueError('No model trained!')
        # print "depgraph size: ", len(depgraphs), ", type: ", type(depgraphs)
        for depgraph in depgraphs:
            # print "transitionparser parse 2"
            conf = Configuration(depgraph, self._user_feature_extractor.extract_features)
            # print "transitionparser parse 3"
            while conf.buffer:
                # print "transitionparser parse 4"
                features = conf.extract_features()
                # print "features type: ", type(features)
                # print "transitionparser parse 5"
                col = []
                row = []
                data = []
                for feature in features:
                    if feature in self._dictionary: #type: dict
                        # if "DIS" in feature:
                        #     print feature.encode("ascii", "ignore")
                ...
                # print "transitionparser parse 6"
                ...
                # print "transitionparser parse 7"
                # Note that SHIFT is always a valid operation
                for y_pred in sorted_predicted_values:
                    # print "transitionparser parse 8"
                    if y_pred in self._match_transition:
                        # print "transitionparser parse 9"
                        strTransition = self._match_transition[y_pred]
                        # print "transitionparser parse 9A"
                        try:
                            baseTransition, relation = strTransition.split(":")
                            # print "transitionparser parse 9B"
                        except ValueError:
                            baseTransition = strTransition
                            # print "transitionparser parse 9BB"

                        if baseTransition == self.transitions.LEFT_ARC:
                            # print "transitionparser parse 9C LEFT_ARC"
                            if self.transitions.left_arc(conf, relation) != -1:
                                # print "transitionparser parse 9CC"
                                break
                        elif baseTransition == self.transitions.RIGHT_ARC:
                            # print "transitionparser parse 9D RIGHT_ARC"
                            if self.transitions.right_arc(conf, relation) != -1:
                                # print "transitionparser parse 9DD"
                                break
                        elif baseTransition == self.transitions.REDUCE:
                            # print "transitionparser parse 9E REDUCE"
                            if self.transitions.reduce(conf) != -1:
                                # print "transitionparser parse 9EE"
                                break
                        elif baseTransition == self.transitions.SHIFT:
                            # print "transitionparser parse 9F SHIFT"
                            if self.transitions.shift(conf) != -1:
                                # print "transitionparser parse 9FF"
                                break
                            # print "transitionparser parse 9FFF"
                        # print "transitionparser parse 9G"
                    else:
                        raise ValueError("The predicted transition is not recognized, expected errors")

                # print "transitionparser parse 10"
            # Finish with operations build the dependency graph from Conf.arcs

            # print "transitionparser parse 13"
            ...
            # print "transitionparser parse 16"

        # print "transitionparser parse 23"
        return result
